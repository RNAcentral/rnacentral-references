...
def main():
    ...
    :return: create xml file in /nfs/references folder
    """
    ...
    Path("/nfs/references").mkdir(parents=True, exist_ok=True)

    # list of xml files in nfs
    path_to_xml_files = Path('/nfs/references')
    ...
    for link in soup.find_all('a'):
        ...
        if file.startswith("PMC") and file not in xml_files:
            get_xml = requests.get(url + file, stream=True)
            with open('/nfs/references/' + file, 'wb') as new_file:
